...
CREATE OR REPLACE FUNCTION shuffled_ints(bigint) RETURNS bigint[] 
	AS '$libdir/bitstring_helpers', 'shuffled_ints' LANGUAGE C STRICT; 
COMMENT ON FUNCTION shuffled_ints(bigint) IS 'Create an array of user definable size ''n'' of shuffled integers (1 to n)';

CREATE OR REPLACE FUNCTION popcount(bit varying) RETURNS int 
	AS '$libdir/bitstring_helpers', 'popcount' LANGUAGE C STRICT IMMUTABLE; 
COMMENT ON FUNCTION popcount(bit varying) IS 'Count the number of 1''s in a bitstring'; 

CREATE OR REPLACE FUNCTION hamming_distance(bit varying,bit varying) RETURNS int 
	AS $$
		SELECT popcount($1 # $2);
...
CREATE OR REPLACE FUNCTION neighbours(bit varying) RETURNS SETOF bit varying 
	AS $$
...
--casts
CREATE OR REPLACE FUNCTION text2bitstring(text) RETURNS bit varying 
	AS $$
...
CREATE OR REPLACE FUNCTION bit2text(bit varying) RETURNS text 
	AS $$
...
/*
CREATE OR REPLACE FUNCTION textarray2bitvaryingarray(text[]) RETURNS bit varying[] 
	AS $$
		DECLARE
  			out bit varying[];
		BEGIN
